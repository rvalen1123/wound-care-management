...
DROP POLICY IF EXISTS "Reps can view doctors" ON doctors;
DROP POLICY IF EXISTS "Reps can view products" ON products;
DROP POLICY IF EXISTS "Reps can view their own orders' payments" ON payments;

-- Drop existing functions with CASCADE
DROP FUNCTION IF EXISTS public.calculate_expected_collection_date(DATE, TEXT) CASCADE;
DROP FUNCTION IF EXISTS public.calculate_units_from_size(TEXT) CASCADE;
DROP FUNCTION IF EXISTS public.orders_auto_calculate() CASCADE;
