...
export const dataService = {
    ...
    try {
      ...
      let query = supabase
        ...
        .select(`
          *,
          doctor:doctors(name),
          product:products(name, manufacturer),
  ...
  async getCommissionStructures(options: QueryOptions = {}): Promise<QueryResult<any>> {
    try {
      ...
      let query = supabase
        ...
        .select(`
          *,
          order:orders(id, date_of_service),
          master_rep:representatives!master_rep_id(name),
          sub_rep:representatives!sub_rep_id(name),
          sub_sub_rep:representatives!sub_sub_rep_id(name)
        `, { count: 'exact' })
  ...
  // Payments
  async getPayments(options: QueryOptions = {}): Promise<QueryResult<any>> {
    try {
      const {
        page = 1,
        limit = 10,
      ...
      let query = supabase
        ...
        .select(`
          *,
          order:orders(id, date_of_service, invoice_to_doc),
          recipient:representatives(name)
        `, { count: 'exact' })
    ...
    try {
      ...
      return {
        data: {
          ...
          totalCommissions: commissionsData?.reduce((sum, comm) => sum + (comm.total_commission || 0), 0) || 0,
          topReps: topReps || []
        },
        error: null
      }
