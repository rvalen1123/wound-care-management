    ...
    <div class="bg-gray-800 rounded-lg p-4 mb-6">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <template v-if="isAdmin">
          <div>
            <label class="block text-sm font-medium text-gray-400">Doctor</label>
            <select
          ...
          <div>
            <label class="block text-sm font-medium text-gray-400">Rep</label>
            <select
              v-model="filters.repId"
              class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-white focus:border-indigo-500 focus:ring-indigo-500"
              @change="handleFilterChange"
            >
        ...
        <table class="min-w-full divide-y divide-gray-700">
          <thead>
            <tr>
              ...
              <th v-if="isAdmin" class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Doctor</th>
              <th v-if="isAdmin" class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Rep</th>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Q Code</th>
          ...
          <tbody class="divide-y divide-gray-700">
            <tr v-for="order in orders" :key="order.order_id" class="hover:bg-gray-700">
              ...
              <td v-if="isAdmin" class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                {{ order.doctor?.name }}
              </td>
              <td v-if="isAdmin" class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                {{ order.master_rep?.name }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                {{ order.q_code }}
              </td>
